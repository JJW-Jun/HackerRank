...
class TestCode {
    ...
    // 이게 테스트 시작전에 가장 먼저 초기화 하는 그런 메서드야
    // 그래서 천만명의 회원을 만들어서  1000만명의 회원을 만들고 for문, 스트림을 속도 비교했어
    @BeforeAll
    ...
    // 우리 프로젝트에서 코드 좀 좋게 짜려다가 for문 / stream속도 테스트를 해봤거든

    // 서버가 병렬로 어차피 처리할 거라서 직렬로 처리했다.
    // 그 병렬 자체를 서버가 해준다. 서버마다 가격이 다르다.

    // 원래 스트림이 제일 빠르다고 해서 찾아봤는데 이게 병렬 처리를 했을때 안 했을때 결과가 너무 차이나서
    @Test
    void 직렬스트림_속도_테스트_streamSpeedTest() {
        for (int a = 0; a < 10; a++) {
            ...
            System.out.println("직렬Stream test: " + (end - start)+ "   result: "+result);
    ...
    void 병렬스트림_속도_테스트_streamSpeedTest() {
        for (int a = 0; a < 10; a++) {
            Long start = System.currentTimeMillis();
            IntStream intStream = storage.stream().parallel().filter(Person -> Person.getAge() < 30).mapToInt(Person::getAge);
            int result = (int) intStream.count();
            Long end = System.currentTimeMillis();
            System.out.println("병렬Stream test: " + (end - start)+ "   result: "+result);
        }
        System.out.println();
    ...
    void for_속도_테스트_streamSpeedTest() {
        for (int a = 0; a < 10; a++) {
            ...
            Long end = System.currentTimeMillis();
            System.out.println("for문 test: " + (end - start)+ "  count"+count);
        }
    }
}
